...
class WC_Product_Helium_Addon
{
	...
	public static function init()
	{
		...
		// ACF
		// add_action('woocommerce_before_add_to_cart_button',  array($self, 'display_custom_field'));
	}
	...
	public function display_custom_field()
	{
		...
		$is_h = get_post_meta($pid, 'helium_checkbox', true);
		$is_h_price = get_post_meta($pid, 'helium_price', true);

		if (($is_h == 'yes') && (isset($is_h_price))) {
			$h_price = "Helium Price Entered ";
		} else {
			$h_price = "Not Entered";
		}
		echo "<div>";
	...
	public static function install()
	{
		...
		add_option('product_helium_addon_message', sprintf(__('Add helium for extra %s ?', 'product-helium-addon-for-woocommerce'), '{price}'));
	}
	...
	public function helium_option_html()
	{
		...
		if ($is_h == 'yes') {
			...
			$cost = get_post_meta($pid, 'helium_price', true);

			// helium weight from product
			// $weight = get_post_meta($post->ID, '_helium_addon_weight', true);;
			$weight = get_post_meta($pid, 'helium_weight', true);
	 ...
	 */
	public function add_cart_item_data($cart_item_meta, $pid)
	{
	...
	public function get_cart_item_from_session($cart_item, $values)
	{
		...
		$cost = get_post_meta($cart_item['product_id'], 'helium_price', true);

		// Get weight from session
		$weight = get_post_meta($cart_item['product_id'], 'helium_weight', true);

		$h_width = get_post_meta($cart_item['product_id'], 'shipping_width', true);
		$h_length = get_post_meta($cart_item['product_id'], 'shipping_length', true);
		$h_height = get_post_meta($cart_item['product_id'], 'shipping_height', true);
		...
		// set new weight
		$cart_item['data']->set_weight($product->get_weight() + $weight);

		// set new weight
		$cart_item['data']->set_height($product->get_height() + $h_height);

		// set new weight
		$cart_item['data']->set_width($product->get_width() + $h_width);

		// set new weight
		$cart_item['data']->set_length($product->get_length() + $h_length);
